...
function createDisplay()
{
	...
	answerRow.classList.add("guess");
	answerRow.id = "guess"+i;
	var addition = addN(i);
	for (var n = 1; n <= 5; n++)
	{
		var value = n + addition;
	    var newBox = document.createElement('div');
		newBox.classList.add("line");
		newBox.id = "line" + value;
        answerRow.appendChild(newBox);
	}
...
function play()
{
	let randomNumber = Math.floor(Math.random() * 5757);
	word = JSON.parse(wordList)[randomNumber];
	console.log(word);
}
...
//         $.ajax({
//             url: 'http://127.0.0.1:5000/get_word_list',
//             type: 'GET',
//             success: function(data) {
//                 console.log('Word List:', data);
...
//         $('#wordList').text(JSON.stringify(wordList, null, 2));
//     }

//     // Fetch the word list when the document is ready or when a button is clicked
//     $('#fetchWords').click(function() {
//         fetchWordList();
//     });

//     // Fetch the word list automatically on page load
//     fetchWordList();
// });
